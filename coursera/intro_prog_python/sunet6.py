...
sunet5.mostrar_bienvenida()
(nombre, edad, estatura_m, estatura_cm, num_amigos) = sunet5.obtener_datos()

print("Muy bien,", nombre, ". Entonces podemos crear un perfil con estos datos.")
sunet5.mostrar_perfil(nombre, edad, estatura_m, estatura_cm, num_amigos)
print("Ya podemos preguntar, recordar y calcular datos. Esperamos que disfrutes con Mi Red")
print("--------------------------------------------------- ")

opcion = 1
while opcion != 0:
    opcion = sunet5.opcion_menu()
    if opcion == 1:
        mensaje = sunet5.obtener_mensaje()
        sunet5.mostrar_mensaje(nombre, None, mensaje)
    elif opcion == 2:
        mensaje = sunet5.obtener_mensaje()
        for i in range(num_amigos):
            nombre_amigo = input("Ingresa el nombre de tu amigo o amiga: ")
            sunet5.mostrar_mensaje(nombre, nombre_amigo, mensaje)
    elif opcion == 3:
        sunet5.mostrar_perfil(nombre, edad, estatura_m, estatura_cm, num_amigos)
    elif opcion == 4:
        nombre = sunet5.obtener_nombre()
        edad = sunet5.obtener_edad()
        (estatura_m, estatura_cm) = sunet5.obtener_estatura()
        num_amigos = sunet5.obtener_num_amigos()
        sunet5.mostrar_perfil(nombre, edad, estatura_m, estatura_cm, num_amigos)
    elif opcion == 0:
